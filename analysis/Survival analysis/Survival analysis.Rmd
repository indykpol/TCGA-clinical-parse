...
```{r}
fredriksson_samples <- read_delim("D:/Git/TCGA-clinical-parse/data/tcga505.samples", "\t", escape_double = FALSE, col_names = c("sample_id", "cancer_type"))
...
clinicalData <- datasets[[1]]@Clinical[,3:5]
clinicalData[is.na(clinicalData[,3]),3] = clinicalData[is.na(clinicalData[,3]),2]
survivalData <- data.frame(Samples=rownames(clinicalData),
                        Time=as.numeric(clinicalData[,3]),
                        Censor=as.numeric(clinicalData[,1]))
```